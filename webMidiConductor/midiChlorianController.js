...
const CHORDS = {
    //Intervals
    MINOR_SECOND_INTERVAL : 'Minor 2nd Interval', //aka Augmented unison
    MAJOR_SECOND_INTERVAL : 'Major 2nd Interval', //aka diminished 3rd
    MINOR_3RD_INTERVAL : 'Minor 3rd Interval', //aka Augmented 2nd, Diminished Power Chord
    MAJOR_3RD_INTERVAL : 'Major 3rd Interval', //aka diminished 4th
    AUGMENTED_3RD_INTERVAL : 'Augmented 3rd Interval', //aka perfect 4th
    DIMINISHED_5TH_INTERVAL : 'Diminished 5th Interval', //aka Augemented 4th
    PERFECT_5TH : 'Perfect 5th Interval', //aka Power Chord, Diminished 6th
    MINOR_6TH_INTERVAL : 'Minor 6th Interval', //aka Stretched Power Chord, Augmented 5th 
    MAJOR_6TH_INTERVAL : 'Major 6th Interval', //aka Diminished 7th
    //CONSTRAINT_Interval_2digitDistance
    //MINOR_7TH_INTERVAL : 'Minor 7th Interval', //aka Augmented 6Th
    //MAJOR_7TH_INTERVAL : 'Major 7th Interval', //aka Diminished Octave
    //Excluding Octave and Augmented Octave for now since midi notes are resolved to base 12 keys and then relative distance is measured to determine step distance. It would not know if pressing (root or octave) or (minor 2nd or augmented octave) 
    //TRIADS
...
//CONSTRAINT_Interval_2digitDistance: Unable to do intervals that have require 2 digit step distance since chord algorith expects a single digit to calculate note combination between each subsequent notes in a chord.
let stepCombinationByChordName = new Map();
stepCombinationByChordName.set(CHORDS.MINOR_SECOND_INTERVAL,1);
stepCombinationByChordName.set(CHORDS.MAJOR_SECOND_INTERVAL,2);
stepCombinationByChordName.set(CHORDS.MINOR_3RD_INTERVAL,3);
stepCombinationByChordName.set(CHORDS.MAJOR_3RD_INTERVAL,4);
stepCombinationByChordName.set(CHORDS.AUGMENTED_3RD_INTERVAL,5);
stepCombinationByChordName.set(CHORDS.DIMINISHED_5TH_INTERVAL,6);
stepCombinationByChordName.set(CHORDS.PERFECT_5TH,7);
stepCombinationByChordName.set(CHORDS.MINOR_6TH_INTERVAL,8);
stepCombinationByChordName.set(CHORDS.MAJOR_6TH_INTERVAL,9);
...
stepCombinationByChordName.set(CHORDS.MINOR_6TH,342);
...
for(midiNoteHz of MIDI_NOTE_NUMBER_MAPPING_INDEX) {
    ...
    setTimeout(() => {
        for (const chordName of stepCombinationByChordName.keys()) {
            loadChordLetterSets(oneNoteObj.midiNoteNumber, stepCombinationByChordName.get(chordName), chordName);
        }
    }, 2000);
}
