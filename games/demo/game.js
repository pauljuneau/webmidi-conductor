 ...
 * BEGIN PONG
 */
//TODO idea: Rename to paddleColorByKeyMap?
/**
 ...
 */
let leftPaddleColorByKeyMap = new Map();
//Acceptable note letters: C, C#, D, D#, E, F, F#, G, G#, A, A#, B"
function loadPaddleColorMap() {
    for(key of ACCEPTABLE_NOTE_NAMES_ARRAY) {
        switch (key) {
            case 'C': //12
                leftPaddleColorByKeyMap.set(key, [255,0,0,1]);//RED
                break; 
            case 'G': //1 
                leftPaddleColorByKeyMap.set(key, [255,128,0,1]);//ORANGE
                break; 
            case 'D': //2
                leftPaddleColorByKeyMap.set(key, [255,255,0,1]);//YELLOW
                break; 
            case 'A': //3 
                leftPaddleColorByKeyMap.set(key, [128,255,0,1]);//Chartreuse
                break; 
            case 'E': //4
                leftPaddleColorByKeyMap.set(key, [0,255,0,1]);//GREEN
                break;     
            case 'B': //5
                leftPaddleColorByKeyMap.set(key, [0,255,128,1]);//SPRING GREEN
                break;
            case 'F#': //6
                leftPaddleColorByKeyMap.set(key, [0,255,255,1]);//CYAN
                break;
            case 'C#': //7 
                leftPaddleColorByKeyMap.set(key, [0,128,255,1]);//DODGER BLUE
                break;  
            case 'G#': //8 Ab on Circle of fifths 
                leftPaddleColorByKeyMap.set(key, [0,0,255,1]);//BLUE
                break;  
            case 'D#': //9 Eb on Circle of fifths
                leftPaddleColorByKeyMap.set(key, [128,0,255,1]);//ELECTRIC INDIGO
                break;  
            case 'A#': //10 Bb on Circle of fifths
                leftPaddleColorByKeyMap.set(key, [255,0,255,1]);//MAGENTA
                break;  
            case 'F': //11
                leftPaddleColorByKeyMap.set(key, [255,0,128,1]);//DEEP PINK
                break;  
            default:
                leftPaddleColorByKeyMap.set(key, [255,0,0,1]);//RED
        }
...
var currentAlphaValue = leftPaddleColorByKeyMap.get(currentKey)[3];
...
function gameSetup() {
    ...
    try {
     ...
     //sets liveAudioInputEnabled to true in pitchdetect.js
...
function loop() {    
    ...
    var red = leftPaddleColorByKeyMap.get(leftPaddleColorKey)[0];
    var green = leftPaddleColorByKeyMap.get(leftPaddleColorKey)[1];
    var blue = leftPaddleColorByKeyMap.get(leftPaddleColorKey)[2];
...
document.addEventListener(MidiInstrumentationEvents.MIDICHLORIANCTRLEVENT, function(e) {
    ...
    //TODO idea:score +1 if in scale; +3 if triad played; +4 if 7th played
